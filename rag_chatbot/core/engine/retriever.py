...
class LocalRetriever:
    ...
    def get_retrievers(
        ...
        # Use simple vector retriever for speed
        # Hybrid + reranking was causing major performance issues (3+ minutes)
        # Simple vector search should be fast (<5 seconds) while still effective
        retriever = self._get_normal_retriever(vector_index, llm, language)
        
        print(f"[RETRIEVER] Using simple vector retriever (top_k={self._setting.retriever.similarity_top_k})")
        return retriever
